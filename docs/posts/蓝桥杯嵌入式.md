...
SysTick挂在系统时钟上，每计数一次，数值减1.数值为0时触发一个中断并重新加载初始值。

* 24位计数器（最大数值为2<sup>24</sup>-1(16777215)
* 向下计数

## 2. 在 HAL 库中的基础用法

如果你只是想用它来做延时，HAL 库已经默认配置好了：

* HAL_Delay(uint32_t Delay)：毫秒级延时。

* 原理：HAL 库默认每 1ms 触发一次 SysTick 中断，在中断服务函数里累加一个全局变量 uwTick。HAL_Delay 就是通过读取这个变量来计算时间的。

## 3.进阶用法：中断回调

如果你想在 SysTick 的每 1ms 中断里做点事（比如扫描按键、刷新数码管），不需要去改底层的汇编文件，HAL 库提供了一个弱定义的函数：

```c
// 在 main.c 或任意文件中重写此函数
void HAL_SYSTICK_Callback(void)
{
    static uint32_t count = 0;
    count++;
    if(count >= 500) // 每 500ms 执行一次
    {
...
* 优先级问题：SysTick 的中断优先级默认是最低的。如果你在另一个高优先级中断里调用 HAL_Delay()，程序会死机。因为高优先级中断阻塞了 CPU，SysTick 中断进不去，uwTick 变量不再增加，HAL_Delay 永远等不到结束。

* 解决：在 CubeMX 的 NVIC 设置里，手动调高 System tick timer 的优先级。

* OS 占用：如果你以后学习 FreeRTOS，SysTick 会被操作系统接管作为“任务切换”的时钟，这时候就不要随便修改它的配置了。
...
| 变量名    | 类型           | 作用                                                         |
| --------- | -------------- | ------------------------------------------------------------ |
| task_func | void (*)(void) | 函数指针。指向具体的任务执行函数。通过调用这个指针来运行任务 |
| rate_ms   | uint32_t       | 任务执行周期。单位毫秒                                       |
| last_run  | uint32_t       | 上次运行时间戳。记录该任务上一次被执行时，任务的具体时间（Tick值） |
...
* task_num:用于存储当前系统中总共有多少个任务
* scheduler_task:一个由task_t结构体组成的静态数组（任务列表）
  * static：关键字表示这个数组的作用域被限制在当前源文件内
  * 数组里面每一行都有一个任务

## 3.任务调度器初始化函数（scheduler_init)
...
这个函数的作用是自动计算任务的总量，并存入task_num中。

* sizeof(scheduler_task):计算整个任务数组占用多少字节的内存。
* sizeof（task_t）：计算单个任务结构体占用多少字节。
* 两者相除就能得出数组中元素的个数（即任务数）。这样在数组中增加或删除任务时会自动适应任务数量，不需要手动更改数字。
...
需要在while（1）中不断循环的执行函数，工作流程如下：

1. 遍历任务：通过for循环，依次检查scheduler_task数组里的每一个任务

2. 获取当前时间：调用HAL_GetTick()获取单片机从启动到现在经过的毫秒数，赋值给局部变量now_time

3. 时间差判断：if(now_time>=周期+上次运行时间）

   * 如果当前时间已经超过设定的触发时间，说明该执行任务了

4. 更新时间戳并执行：

   * scheduler_task[i].last_run = now_time；把当前时间记录为最后一次运行时间，为下一次计算做准备
   * scheduler_task[i].task_func()；通过函数指针调用对应的实际任务函数
