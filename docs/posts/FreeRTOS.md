...
与二进制信号量不同的是，如果消费者任务一次消费时间比较长，生产者任务可以多次释放信号，计数信号量便可以记录下来释放了多少次，灯消费者任务消费完成后再继续获取信号量，消费相应的次数。不会像二进制信号量那样，当有信号等待获取时，就忽略掉后续的释放。

计数信号量，在实际的工程中通常有两种工作环境，**盘点事件**与**资源管理**。
盘点事件：记录下事件发生的次数，进行相应次数的处理。
资源管理：使用计数信号量来表示系统中某种资源的可用数量，在程序初始化时就将计数信号量初始化为此资源的数量，当有一个任务需要使用此种资源时，便使用osSemaphoreAcquire函数获取信号量，这是计数信号量的数量-1，然后此任务便可以对一个此资源进行使用，如果有多个任务使用此资源，那么每个任务都会对此信号量进行获取，减少信号量的计数值，当信号量减少为0，后面的任务获取不到信号量，进入阻塞态等待；当前面的任务使用完了此资源，可以使用osSemapgoreRelease函数“释放”信号量，在阻塞态等待的任务就可以被唤醒，获取到资源进行使用。
