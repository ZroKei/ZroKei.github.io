...
队列（Queue）：队列就是一条数据运输的管道，产生数据的任务，我们称为生产者，它可以将数据塞入到管道中排队，而需要这个数据的任务我们称为消费者，它可以从队列中取出数据使用。生产者产生了多少条数据，消费者就只能取出多少条数据，所以就不会出现事件重复的问题。而且由于队列有一定的长度可以排队缓存数据，因此便不会出现因消费者在处理数据时无暇顾及而导致生产者产生的数据被忽略的情况
