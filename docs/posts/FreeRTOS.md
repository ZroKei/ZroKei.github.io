...
资源管理：使用计数信号量来表示系统中某种资源的可用数量，在程序初始化时就将计数信号量初始化为此资源的数量，当有一个任务需要使用此种资源时，便使用osSemaphoreAcquire函数获取信号量，这是计数信号量的数量-1，然后此任务便可以对一个此资源进行使用，如果有多个任务使用此资源，那么每个任务都会对此信号量进行获取，减少信号量的计数值，当信号量减少为0，后面的任务获取不到信号量。
